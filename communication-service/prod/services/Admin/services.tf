...
resource "aws_lb_listener_rule" "service_targetGroup_listnerRule" {
  ...
  action {
    type             = "forward"
    target_group_arn = aws_lb_target_group.admin-target_group_private.arn
  }
...
resource "aws_ecs_service" "admin_service_ecs" {
  ...
  load_balancer {
    target_group_arn = aws_lb_target_group.admin-target_group.arn
...
resource "aws_appautoscaling_target" "ecs_target_autoscale" {
  ...
  depends_on = [
    aws_ecs_service.admin_service_ecs
  ]
}
