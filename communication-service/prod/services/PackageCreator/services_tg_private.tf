...
resource "aws_lb_target_group" "pkg-create-target_group_private" {
  name        = "pkg-create-tg-pri-${var.env}"
  port        = 80
  protocol    = "HTTP"
  target_type = "ip"
  vpc_id      = var.vpcId
...
resource "aws_lb_listener_rule" "listener_rule_private" {
  ...
  action {
    type             = "forward"
    target_group_arn = aws_lb_target_group.pkg-create-target_group_private.id
  }
