...
resource "aws_lb_listener_rule" "service_targetGroup_listnerRule" {
  ...
  action {
    type             = "forward"
    target_group_arn = aws_lb_target_group.pkg-trans-target_group.arn
  }
...
resource "aws_ecs_service" "pkg-trans_service_ecs" {
  ...
  network_configuration {
    ...
    security_groups  = var.pkg_trans_sg
  ...
  load_balancer {
    #target_group_arn = aws_lb_target_group.pkg-trans-target_group_private.arn
    target_group_arn = aws_lb_target_group.pkg-trans-target_group_private.arn
    container_name   = var.serviceName
    container_port   = var.conatiner_port
  }
  ...
  depends_on = [
    aws_lb_target_group.pkg-trans-target_group,
    aws_lb_target_group.pkg-trans-target_group_private
  ]
}
...
resource "aws_appautoscaling_target" "ecs_target_autoscale" {
  ...
  depends_on = [
    aws_ecs_service.pkg-trans_service_ecs
  ]
}
