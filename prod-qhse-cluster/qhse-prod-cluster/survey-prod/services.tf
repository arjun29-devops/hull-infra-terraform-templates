...
resource "aws_lb_listener_rule" "service_targetGroup_listnerRule" {
  ...
  action {
    type             = "forward"
    target_group_arn = aws_lb_target_group.qhse-survey-target_group.arn
  }
...
resource "aws_ecs_service" "qhse-survey_service_ecs" {
  ...
  depends_on = [
    aws_lb_target_group.qhse-survey-target_group
  ]
}
...
resource "aws_appautoscaling_target" "ecs_target_autoscale" {
  ...
  depends_on = [
    aws_ecs_service.qhse-survey_service_ecs
  ]
}

resource "aws_ecr_repository" "ecr" {
  name = "${ var.ProjectName }-survey-${ var.env }"
}