...
resource "aws_lb_listener_rule" "service_targetGroup_listnerRule" {
  ...
  action {
    type             = "forward"
    target_group_arn = aws_lb_target_group.qhse-broadcast-target_group.arn
  }
...
resource "aws_ecs_task_definition" "service_task_defination" {
  ...
  container_definitions = file("${var.serviceName}-${var.env}.json")
...
resource "aws_ecs_service" "qhse-broadcast_service_ecs" {
  ...
  depends_on = [
    aws_lb_target_group.qhse-broadcast-target_group
  ]
}
...
resource "aws_appautoscaling_target" "ecs_target_autoscale" {
  ...
  depends_on = [
    aws_ecs_service.qhse-broadcast_service_ecs
  ]
}

resource "aws_ecr_repository" "ecr" {
  name = "${ var.ProjectName }-broadcast-${ var.env }"
}