...
terraform {
  backend "s3" {
    ...
    key     = "services/voyage-optimization-map-api/terraform.tfstate"
    region  = "ap-south-1"
    profile = "hull-dev-test"
  }
}
 ...
 #resource "aws_ecs_cluster" "this" {
  #name = "voyage-optimization-common-apps-ecs-cluster-qa"
#}
module "ecs" {
  cluster_name              = "voyage-optimization-common-apps-ecs-cluster-qa"
  ...
  target_group_health_check = "/api/vo/Health/HealthCheck"
  ...
  alb_path_based_listener   = "/api/vo/*"
 # depends_on                = [aws_ecs_cluster.this]
  create_ecr_repo           = true
}

locals {
  project_name = "voyage-optimization-map-api"
  env          = "qa"
}
