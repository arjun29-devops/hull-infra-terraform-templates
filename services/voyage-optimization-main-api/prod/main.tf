...
terraform {
  backend "s3" {
    ...
    key     = "services/ocean-eye-voyage-optimization-main-api-prod/terraform.tfstate"
    region  = "ap-south-1"
    profile = "ocean-eye-prod"
  }
}
...
module "ecs" {
  ...
  target_group_health_check = "/api/vm/Health/HealthCheck"
  ...
  alb_path_based_listener   = "/api/vm/*"
  #depends_on                = [aws_ecs_cluster.this]
  create_ecr_repo           = true
}
locals {
  project_name = "voyage-main-api"
  env          = "prod"
}
