...
terraform {
  backend "s3" {
    ...
    key     = "services/ocean-eye-propulsion-pro-ticketing-api-prod/terraform.tfstate"
    region  = "ap-south-1"
    profile = "ocean-eye-prod"
  }
}
...
module "ecs" {
  ...
  target_group_health_check = "/api/ppt/home/health"
  ...
  alb_path_based_listener   = "/api/ppt/*"
  #depends_on                = [aws_ecs_cluster.this]
  create_ecr_repo           = true
}
locals {
  project_name = "pro-ticketing-api"
  env          = "prod"
}
