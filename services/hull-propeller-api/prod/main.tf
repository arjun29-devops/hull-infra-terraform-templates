...
terraform {
  backend "s3" {
    ...
    key     = "services/ocean-eye-propeller-api-prod/terraform.tfstate"
    region  = "ap-south-1"
    profile = "ocean-eye-prod"
  }
}
...
module "ecs" {
  ...
  target_group_health_check = "/api/hp/home/health"
  ...
  alb_path_based_listener   = "/api/hp/*"
  #depends_on                = [aws_ecs_cluster.this]
  create_ecr_repo           = true
}
locals {
  project_name = "ocean-eye-propeller-api"
  env          = "prod"
}
