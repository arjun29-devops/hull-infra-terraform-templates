...
terraform {
  backend "s3" {
    ...
    key     = "services/voyage-fleet-monitoring/terraform.tfstate"
    region  = "ap-south-1"
    profile = "hull-dev-test"
  }
}
...
module "ecs" {
  ...
  target_group_health_check = "/api/vf/Health/HealthCheck"
  ...
  alb_path_based_listener   = "/api/vf/*"
 # depends_on                = [aws_ecs_cluster.this]
  create_ecr_repo           = true
}

locals {
  project_name = "voyage-fleet-monitoring-api"
  env          = "qa"
}
