...
terraform {
  backend "s3" {
    ...
    key     = "services/voyage-fleet-monitoring-api/terraform.tfstate"
    region  = "ap-south-1"
    profile = "hull-dev-test"
  }
}
...
#resource "aws_ecs_cluster" "this" {
 # name = "voyage-optimization-common-apps-ecs-cluster-dev"
#}
module "ecs" {
  cluster_name              = "voyage-optimization-common-apps-ecs-cluster-dev"
...
locals {
  project_name = "voyage-fleet-monitoring"
  env          = "dev"
}
