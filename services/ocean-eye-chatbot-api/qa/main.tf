...
terraform {
  backend "s3" {
    ...
    key     = "services/ocean-eye-chatbot-api/terraform.tfstate"
    region  = "ap-south-1"
    profile = "hull-dev-test"
  }
}
...
#}
module "ecs" {
  cluster_name              = "hull-common-apps-ecs-cluster-qa"
  ...
  target_group_health_check = "/api/Health/HealthCheck"
  ...
  alb_path_based_listener   = "/api/v1/*"
 # depends_on                = [aws_ecs_cluster.this]
  create_ecr_repo           = true
}

locals {
  project_name = "ocean-eye-chatbot-api"
  env          = "qa"
}
